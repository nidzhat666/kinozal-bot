...
def _build_season_queries(clean_titles: list[str], season_number: int, year: int | str | None) -> set[str]:
...
async def perform_torrent_search(
    ...
    *,
    ...
    tasks_with_info = [
        (provider, q, provider.search(q, requested_item=requested_item, requested_type=requested_type))
        for q in queries
    ...
    try:
        ...
        for (provider, query, _), res in zip(tasks_with_info, results_list):
            ...
            if isinstance(res, Exception):
                ...
                search_logger.info("Found torrents", provider=provider_name, query=query, count=count)
        
        for provider_name, total_count in provider_stats.items():
            search_logger.info("Total results across all queries", provider=provider_name, total_count=total_count)
...
def _is_audio_release(result_name: str) -> bool:
    ...
    strict_audio_keywords_substring = ["audio pack", "[audio pack]", "soundtrack", "саундтрек"]
    conditional_audio_keywords = [
        "аудиодорож", "озвучк", "mp3", "flac", "albums", "album", "tracks", "track",
        "lossless", "аудио", "музыка",
    ]
...
def _is_disc_image(result_name: str) -> bool:
    ...
    result_lower = result_name.lower()
    disc_keywords = ["dvd9", "dvd-9", "dvd5", "dvd-5", "ntsc", "pal", "2 x dvd-9", "2 x dvd9"]
...
def _has_multiple_resolutions(result_name: str) -> bool:
    ...
    found_resolutions = [name for name, pattern in resolutions.items() if re.search(pattern, result_lower)]
    return len(found_resolutions) > 1 and "1080p" in found_resolutions and "720p" in found_resolutions
...
def _get_expected_year(media_details: MediaDetails | None, season_number: int | None, season_year: int | None) -> int | None:
...
def _should_skip_result(
    ...
    if (result.video_quality is None or result.video_quality == "Unknown") and not _has_video_markers(result_name):
...
def _filter_and_process_results(
    ...
    for result in raw_results:
        ...
        should_skip, reason = _should_skip_result(
            result, result_name, provider_name, media_details, season_number, expected_year, expected_titles
        )
        ...
        if should_skip:
            if reason:
                ...
                logger.debug(
                    ...
                    name=result_name,
                    provider=provider_name,
                    season_number=season_number if reason == "season_pack" or reason == "season_mismatch" else None,
...
def _log_quality_stats(quality_counter: Counter[str], total_raw: int, total_filtered: int) -> None:
    ...
    if not quality_counter:
        quality_logger.info("No torrents found after filtering", total_raw=total_raw, total_filtered=0)
        return
...
def _is_fuzzy_match(result_name: str, expected_titles: list[str]) -> bool:
    ...
    for expected in expected_titles:
        ...
        sections = re.split(r'\s*[/:]\s*', result_clean, 1)
        if sections:
            ...
            if len(first_section_words) >= len(expected_words):
                if first_section_words[:len(expected_words)] == expected_words:
                    return True
...
def _sort_and_group_results(results: list[MovieSearchResult]) -> list[MovieSearchResult]:
    ...
    best = [max(group, key=lambda r: r.seeds or -1) for _, group in groupby(results, key=quality_key)]
...
def format_torrent_search_results(
    ...
    *,
    ...
    if back_callback_key:
        buttons.append([
            InlineKeyboardButton(text=back_button_text or "Назад", callback_data=back_callback_key)
        ])
