...
async def call_plex(library_id: int) -> bool:
    ...
    async with ClientSession() as session:
        async with session.get(get_url(f"/library/sections/{library_id}/refresh"), headers=headers) as response:
...
async def refresh_plex_library():
    ...
    return "Plex libraries refreshed successfully." if any(results) else "Failed to refresh Plex libraries."
